...
function main() {
    const routes = checkGeneratedLegacyRoutes()
    ...
    for (const sitemapPath of SITEMAP_CANDIDATES) {
        ...
        for (const route of routes) {
            if (sitemap.includes(`/${route.sourceSlug}`)) {
    ...
    console.log(`[check:legacy-routes] OK: ${routes.length} legacy route(s) have valid targets, redirects, and sitemap exclusions`)
}
