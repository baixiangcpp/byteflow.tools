...
describe("verification mode acceptance guard", () => {
    it("records only request hosts and storage keys, never sensitive payload fields", () => {
        ...
        expect(panelSource).toContain("verification_mode_scope")
        ...
        expect(trustSource).toContain("VERIFICATION_STORAGE_KEY_POLICY")
        expect(trustSource).toContain('"byteflow:tools:favorites"')
        expect(trustSource).toContain('"payload"')
        expect(trustSource).not.toContain('"byteflow:"')
