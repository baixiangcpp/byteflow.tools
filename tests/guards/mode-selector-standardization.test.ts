...
describe("tool mode selector standardization", () => {
    it("keeps high-traffic mode toggles on the shared ModeSelector", () => {
        ...
        expect(selectorSource).toContain("role=\"radio\"")
        expect(selectorSource).toContain("aria-checked={active}")
        expect(selectorSource).toContain("ArrowRight")
        expect(selectorSource).toContain("ArrowLeft")
        expect(selectorSource).toContain("Home")
        expect(selectorSource).toContain("End")
    })
})
