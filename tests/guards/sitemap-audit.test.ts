...
describe("BF-047 sitemap audit", () => {
    ...
    it("wires export-level sitemap QA into build checks", () => {
        ...
        expect(exportGate).toContain("expected one canonical tag")
        expect(exportGate).toContain("duplicate HTML hreflang values")
        ...
        expect(exportGate).toContain("sitemap loc is not expected canonical")
        expect(exportGate).toContain("parameterizedUrlsAllowed: false")
    })

    it("keeps reliable lastmod manifest checks in validation before sitemap export QA", () => {
        const packageJson = JSON.parse(fs.readFileSync(path.join(process.cwd(), "package.json"), "utf8"))
        const lastmodGate = fs.readFileSync(path.join(process.cwd(), "scripts/gates/check-sitemap-lastmod.js"), "utf8")
        const generator = fs.readFileSync(path.join(process.cwd(), "scripts/lib/sitemap-lastmod-lib.js"), "utf8")

        expect(packageJson.scripts.validate).toContain("npm run check:sitemap-lastmod")
        expect(packageJson.scripts.build).toMatch(/npm run validate && npm run build:app && npm run build:post/)
        expect(lastmodGate).toContain("buildSitemapLastmodManifest")
        expect(lastmodGate).toContain("sitemap lastmod manifest is stale")
        expect(generator).toContain('execFileSync("git"')
        expect(generator).toContain('"log", "-1", "--format=%cI"')
        expect(generator).toContain("src/app/[lang]")
        expect(generator).toContain("loadOrderedToolManifests")
        expect(generator).toContain("manifest.sourceFile")
        expect(generator).toContain("includeToolMeta: true")
    })
})
