...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const locale = lang;
    return buildToolMetadata({ lang: locale, slug: "chmod-calculator" });
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
    ...
    return (
        <>
            <ToolBreadcrumbJsonLd lang={locale} slug="chmod-calculator" />
            {children}
            <ToolContentTemplateServer toolSlug="chmod-calculator" lang={locale} />
        </>
    );
}
