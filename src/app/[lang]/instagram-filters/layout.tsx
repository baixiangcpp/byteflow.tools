...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const locale = lang
    return buildToolMetadata({ lang: locale, slug: "instagram-filters" })
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
    ...
    return (
        <>
            <ToolBreadcrumbJsonLd lang={locale} slug="instagram-filters" />
            {children}
            <ToolContentTemplateServer toolSlug="instagram-filters" lang={locale} />
        </>
    )
}
