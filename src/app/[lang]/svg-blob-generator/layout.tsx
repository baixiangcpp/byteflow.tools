...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const locale = lang
    return buildToolMetadata({ lang: locale, slug: "svg-blob-generator" })
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
    ...
    return (
        <>
            <ToolBreadcrumbJsonLd lang={locale} slug="svg-blob-generator" />
            {children}
            <ToolContentTemplateServer toolSlug="svg-blob-generator" lang={locale} />
        </>
    )
}
