...
import { Copy, Download, Eraser, ImageDown, ShieldCheck, TestTube2, Youtube } from "lucide-react"
...
export function YouTubeThumbnailGrabberPage() {
    const { t } = useLang()
    const toolT = t.tools["youtube_thumbnail_grabber"] as Record<string, string>
    const noneLabel = t.common.none
    const statusIdle = (t.common.thumbnail_status_idle).replace("{platform}", "YouTube")
    const statusInvalid = (t.common.thumbnail_status_invalid).replace("{platform}", "YouTube")
    ...
    React.useEffect(() => {
        ...
        const next = buildYouTubeThumbnailCandidates(id)
        setVideoId(id)
        setCandidates(next)
    ...
    const handleDownload = () => {
        ...
        const filename = `youtube-thumbnail-${videoId || "image"}.jpg`
